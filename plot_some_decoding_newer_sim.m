function [Posterior_all] = plot_some_decoding_newer_sim( ...
    animal, iseries, exp_list, es,...
    type, only_correct, smthWin, box_filt, quickProcess, shank_list, area)
...
% load(['./Data/es_model_common_exp_' num2str(idx)]);
...
if ~test_normal
    ...
    [ML_norm] = dec.predictBayesDecoder(spkRate(t,:), 0,'best');
...
%         tmpDec_high = dec_h;
        ...
        for icell = 1:size(spkRate,2)
            tmpSpkRate = spkRate;
            tmpSpkRate(:,icell) = NaN;
            [tmpML_low ] = dec.predictBayesDecoder(spkRate(t_low,:), 0,'best');
            [tmpML_high] = dec.predictBayesDecoder(spkRate(t_high,:), 0,'best');
            [tmpML_norm] = dec.predictBayesDecoder(spkRate(t,:), 0,'best');
            ...
            %             tmpMap_high = tmpMap;
            tmpMap_low.CVO = [];
            tmpMap_low = tmpMap_low.trainSpikeMap(tmpML_low', spkRate(t_low,icell), 0);
            
            tmpMap_high.CVO = [];
            tmpMap_high = tmpMap_high.trainSpikeMap(tmpML_high', spkRate(t_high,icell), 0);
            
            tmpMap_norm.CVO = [];
            tmpMap_norm = tmpMap_norm.trainSpikeMap(tmpML_norm', spkRate(t,icell), 0);
            
            dec_l_d.model.EV(:,icell) = tmpMap_low.model.EV;
            dec_h_d.model.EV(:,icell) = tmpMap_high.model.EV;
            dec_n_d.model.EV(:,icell) = tmpMap_norm.model.EV;
            ...
            for tmpIdx = 1:tmpMap.kfold
                dec_l_d.model.trained(tmpIdx).respModel_orig(icell,:) = tmpMap_low.model.tuning.respModel(tmpIdx,:);
                ...
                dec_h_d.model.trained(tmpIdx).respModel_orig(icell,:) = tmpMap_high.model.tuning.respModel(tmpIdx,:);
                ...
                dec_n_d.model.trained(tmpIdx).respModel_orig(icell,:) = tmpMap_norm.model.tuning.respModel(tmpIdx,:);
...
Posterior_all.confidence.low = (max((2.^Posterior_all.Posterior_low)') - min((2.^Posterior_all.Posterior_low)'))/50;
Posterior_all.confidence.norm = (max((2.^Posterior_all.Posterior_norm)')- min((2.^Posterior_all.Posterior_norm)'))/50;
Posterior_all.confidence.high = (max((2.^Posterior_all.Posterior_high)') - min((2.^Posterior_all.Posterior_high)'))/50;
Posterior_all.confidence.gray = (max((2.^Posterior_all.Posterior_gray)') - min((2.^Posterior_all.Posterior_gray)'))/50;
...
Posterior_all.fieldWidths.high_decPos = 2*(Posterior_all.decoder.numBins - calculate50crosses(Posterior_all.decoder_high_decPos.model.bestModel));
Posterior_all.fieldWidths.low_decPos =  2*(Posterior_all.decoder.numBins - calculate50crosses(Posterior_all.decoder_low_decPos.model.bestModel));
Posterior_all.fieldWidths.norm_decPos =  2*(Posterior_all.decoder.numBins - calculate50crosses(Posterior_all.decoder_norm_decPos.model.bestModel));
