...
%     if box_filt
%         spkRate_orig(:,icell) = smthInTime(es.orig.spikeTrain(:,icell), 60, smthWin,'box');
%     else
%         spkRate_orig(:,icell) = smthInTime(es.orig.spikeTrain(:,icell), 60, smthWin); %*(smthWin*60./1000);
%     end
end
...
dec_l_d.numBins = numSampleBins;
dec_h_d = bayesDecoder;%dec;
dec_h_d.numBins = numSampleBins;
dec_n_d = bayesDecoder;%dec;
dec_n_d.numBins = numSampleBins;

% end
if only_correct
    dec2 = bayesDecoder;%dec;
    dec2.numBins = numSampleBins;
    [dec, ~, ~, ~] = ...
...
%             [tmpML_high] = dec.predictBayesDecoder(tmpSpkRate(t_high,:), 0,'best');
            ...
            [tmpMap_low] = tmpMap_low.trainSpikeMap(tmpML_low(X_low_orig'<rewlim & tmpML_low<rewlim)', spkRate(t_low & temp<rewlim & temp2<rewlim,icell), 0);
...
%             [tmpMap_high] = tmpMap_high.trainSpikeMap(tmpML_high(X_high_orig<rewlim)', spkRate(t_high & temp<rewlim,icell), 0);
            ...
            [tmpMap_high] = tmpMap_high.trainSpikeMap(tmpML_high(X_high_orig'<rewlim & tmpML_high<rewlim)', spkRate(t_high & temp<rewlim & temp2<rewlim,icell), 0);
...
%             [tmpMap_norm] = tmpMap_norm.trainSpikeMap(tmpML_norm(X_norm<rewlim)', spkRate(t& temp<rewlim,icell), 0);
            temp2 = zeros(size(t));
            temp2(t) = tmpML_norm;
            [tmpMap_norm] = tmpMap_norm.trainSpikeMap(tmpML_norm(X_norm'<rewlim & tmpML_norm<rewlim)', spkRate(t & temp<rewlim & temp2<rewlim,icell), 0);
            
            tmpMap_low_orig.CVO = [];
            temp = zeros(size(t_low));
            temp(t_low) = X_low_orig;
            [tmpMap_low_orig] = tmpMap_low_orig.trainSpikeMap(X_low_orig(X_low_orig<rewlim)', spkRate(t_low & temp<rewlim,icell), 0);
            
            tmpMap_high_orig.CVO = [];
            temp = zeros(size(t_high));
            temp(t_high) = X_high_orig;
            [tmpMap_high_orig] = tmpMap_high_orig.trainSpikeMap(X_high_orig(X_high_orig<rewlim)', spkRate(t_high & temp<rewlim,icell), 0);
           
            tmpMap_norm_orig.CVO = [];
            temp = zeros(size(t));
            temp(t) = X_norm;
            [tmpMap_norm_orig] = tmpMap_norm_orig.trainSpikeMap(X_norm(X_norm<rewlim)', spkRate(t& temp<rewlim,icell), 0);
