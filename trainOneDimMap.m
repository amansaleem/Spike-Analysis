function [obj, Prediction, V] = trainOneDimMap(obj, V, Y, smth_win, smthWin)
...
%     obj.CVO = crossValPartition_MD(obj.CVO, ones(1,length(V)), 1:length(V), obj.kfold);
end
...
pred_temp = zeros(obj.CVO.kfold, size(Y,2), sum(obj.CVO.test{1}));
...
%     respModel = zeros(size(Y,2),obj.numBins);
    for icell = 1:size(Y,2)
        [model, pred, win_ratio] = get1Dmap(Y(:,icell), V', obj.numBins, obj.bins, obj.CVO, iter, obj.sampleRate, obj.smth_win, smthWin);
        ...
        obj.win(iter,icell) = win_ratio;
        pred_temp(iter,icell,:) = pred;
    end
    obj.model.EV(iter,obj.model.EV(iter,:)<0) = 0;
end
%%
if size(pred_temp,2)>1
    for iter = 1:size(pred_temp,1)
        Prediction = [Prediction squeeze(pred_temp(iter,:,:))];
    end
else
    for iter = 1:size(pred_temp,1)
        Prediction = [Prediction squeeze(pred_temp(iter,:,:))'];
