function [expt,exptfp] = ExptLoadCerebusTracesFPs(varargin)
...
%
% [expt,exptfp] = ExptLoadCerebusTracesFPs(ExptTag) returns the raw traces
% in expt and the LFP traces  in exptfp for experiment ExptTag (with fields
% animal, iseries, iexp). The raw traces are downsampled by a factor of 2
% relative to their original acquisition rate. The LFP traces are sampled
% at 400 Hz (unless specified differently, see below). It creates two files
% - one for expt and one for exptfp so if you run it one more time it just
% loads the files.
%
% In all calls, ExptTag can be replaced by three arguments: animal,iseries,iexp.
% 
% [expt,exptfp] = ExptLoadCerebusTracesFPs(ExptTag,refChan) Lets you
% specify an internal reference electrode. This channel is the one after
% the probe has been unmapped, i.e., 3 means to reference from channel 3 -
% the third from the top in an A1x16 michigan probe. Default: [].
%
% [expt,exptfp] = ExptLoadCerebusTracesFPs(ExptTag,[], [preStimTime,
% postStimTime]) Lets you specify a preStimTime and postStimTime. Default:
% [0 0].
%
% [expt,exptfp] = ExptLoadCerebusTracesFPs(ExptTag,[], [], lfprate) Lets
% you specify the samplerate for the field potentials in Hz. Default: 400.
%
% [expt,exptfp] = ExptLoadCerebusTracesFPs(ExptTag,[], [], [], ,LoadWhat) lets
...
% MC 2012-01-19 Option to save only LFP. Updated help and parsing of arguments
%

msgbox('ExptLoadCerebusTracesFPs is obsolete. Please switch to ExptLoadCerebusTraces (which also saves the field potentials) or send an email to Matteo so he can make the appropriate changes. Thanks.');
...
if nargin >= NextArg
    refChan = varargin{NextArg};
end

if nargin >= NextArg+1
    if ~isempty(varargin{NextArg+1})
        preStimTime   = varargin{NextArg+1}(1);
        postStimTime  = varargin{NextArg+1}(2);
    end
end

if nargin >= NextArg+2
    if ~isempty(varargin{NextArg+2})
        lfpsamplerate = varargin{NextArg+2};
    end
end
   
if nargin >= NextArg+3
    LoadWhat = varargin{NextArg+3};
...
if exist(FileNameFP,'file')==2
    ...
    fprintf('done\n');
    if abs(exptfp.samplerate/lfpsamplerate -1) > 0.05 % if you are off by > 5 percent of desired samplerate
        clear exptfp;
    end
end
...
if strcmp(LoadWhat,'LFPonly') && ~overwrite_flag % the job is done
    expt = [];
    return
end

% this has a bug -- returns and therefore does not compute exptfp...
