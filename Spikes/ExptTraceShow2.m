function u = ExptTraceShow2( expt, gain, offset, type, show)
...
% 2011-07 BH added input argument type, for loading lfp (type=1) or
%         vm(type=2)
% 2011-08 BH added input argument type == 3 for loading membrane current
...
for istim = 1:expt.nstim
    ...
    for irpt = 1:expt.nrepeats
        ...
        if ~isempty(expt.data{type}{istim, irpt})
%             vv = double(expt.data{1}{istim, irpt});   
%             vv = double(expt.data{1}{istim, irpt}); % for filtered LFP on Axon secondary, AI4  BH 10.07.10
            vv = double(expt.data{type}{istim, irpt}); % for Vm on Axon primary, AI6  BH 5.18.11
