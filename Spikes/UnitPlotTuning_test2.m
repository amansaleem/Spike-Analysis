function fig = UnitPlotTuning_test( unit, protocol, graphinfo, fig, AnalysisType , binstate, colorin)
...
% 2011-06 AS added binstate
% 2011-09 AS added the color option
...
case 'Mean'
    if flag_state
        [yy, ee] = UnitGetDC_test( unit, errflag, 0, binstate );
    else
        [yy, ee] = UnitGetDC( unit, errflag, 0);
    end
case 'First Harmonic'
...
%       ax(irow,icol) = gridplot(nrows,ncols,irow,icol);
      ...
      if length(unique(x)) == length(x)
         ...
         pp = errorbar(x,y,e,strcat(colorin, 's-'));
         set(pp,'markerfacecolor',colorin);
         
      else
         pp = errorbar(x,y,e,strcat(colorin, 's')); hold on
         set(pp,'markerfacecolor',colorin);
         xvalues = unique(x);
         yvalues = [];
         for xvalue = xvalues
         ...
         plot(xvalues,yvalues,strcat(colorin, '-')); 
      end
      hold on;
      if ~isempty(protocol.blankstims)
         plot([min(xx) max(xx)], mean(yy(protocol.blankstims))*[1 1],strcat(colorin, '--'))
      end
