...
for iax = 1:nax
    ...
    if nargin>1
        try
            ...
            %plot([1;1]*protocol.pars(istim),[0 nrpts],'g-');
            plot([1;1]*protocol.pars(1,istim)/10,[0 nrpts],'g-');%dur = 10* stim duration in sec
...
title(rax(1),[ unit.id ' Expt ' num2str(unit.iseries) '-' num2str(unit.iexp) ]);
