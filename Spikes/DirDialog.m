...
% 2011-01 AS: further split animals into subsets based on filename Line 78 & 170-200.
...
fdialogh.popSubset = uicontrol('Style','popupmenu','Units','Pixels','Tag','popSubset',...
    'Position',[220 15 120 20],'String',{ 'All animals'; 'all Cats'; 'all Mice'; 'only BALL'; 'only TS'; 'only ZINTRA'; 'only ND'},...
            ...
            if ~isempty(strarrAnimals)
                switch get(fdialogh.popSubset,'Value')
                    ...
                    case 3 % "all Mice"
                        ...
                        strarrAnimals(find(notADir)) = [];
                        strarrAnimals = struct2cell(strarrAnimals);
                        % AS: 21/1/11 Added the function to further split
                        % animals based on filename.
                    case 4 % "only BALL"
                        notADir = zeros(length(strarrAnimals),1);
                        for i = 1:length(notADir)
                            if isempty(strfind(strarrAnimals{1,i},'BALL'))
                                notADir(i) = 1;
                            end
                        end
                        strarrAnimals = cell2struct(strarrAnimals,'name');
                    ...
                    case 5 % "only TS"
                        notADir = zeros(length(strarrAnimals),1);
                        for i = 1:length(notADir)
                            if isempty(strfind(strarrAnimals{1,i},'TS'))
                                notADir(i) = 1;
                            end
                        end
                        strarrAnimals = cell2struct(strarrAnimals,'name');
                    ...
                    case 6 % "only ZINTRA"
                        notADir = zeros(length(strarrAnimals),1);
                        for i = 1:length(notADir)
                            if isempty(strfind(strarrAnimals{1,i},'ZINTRA'))
                                notADir(i) = 1;
                            end
                        end
                        strarrAnimals = cell2struct(strarrAnimals,'name');
                    ...
                    case 7 % "only ND"
                        notADir = zeros(length(strarrAnimals),1);
                        for i = 1:length(notADir)
                            if isempty(strfind(strarrAnimals{1,i},'ND'))
                                notADir(i) = 1;
                            end
                        end
                        strarrAnimals = cell2struct(strarrAnimals,'name');
