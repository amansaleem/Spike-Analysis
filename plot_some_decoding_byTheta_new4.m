function [Posterior_all] = plot_some_decoding_byTheta_new4(animal, iseries,...
...
% end
if (iseries)==531
    load(['Data' filesep 'es_' animal '_' num2str(iseries) '_thetaBinsA_new']);
else
    load(['Data' filesep 'es_' animal '_' num2str(iseries) '_thetaBinsB_new']);
end
...
trialEnds = [find(diff(es.trialID)>=1)-2];
temp = zeros(size(es.traj));
trialEnds(trialEnds<0) = 1;
temp(trialEnds) = 1;
trialEnds = temp;
