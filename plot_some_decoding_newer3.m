function [Posterior_all] = plot_some_decoding_newer3(animal, iseries,...
    exp_list, type, only_correct, smthWin, box_filt, quickProcess, shank_list, area)
% Newer3 basically limits the lower limit to -5.6
...
if only_correct
    ...
    Posterior_norm = (2.^Posterior_norm)/50;
else
...
if ~test_normal
    ...
    Posterior_high_orig =[];
    Posterior_low_orig  =[];
    Posterior_low  = (2.^Posterior_low)/50;
    Posterior_high = (2.^Posterior_high)/50;
else
    [dec_l, ~, X_low_orig, Posterior_low_orig] = ...
        dec_l.trainDecoder( es.traj(t_low), spkRate(t_low,:), 0);
    [dec_h, ~, X_high_orig, Posterior_high_orig] = ...
        dec_h.trainDecoder( es.traj(t_high), spkRate(t_high,:), 0);
    [ML_low,  Posterior_low] = dec.predictBayesDecoder(spkRate(t_low,:), 0,'mean');
    [~,  Posterior_gray] = dec.predictBayesDecoder(spkRate(t_gray,:), 0,'mean');
    [ML_high,  Posterior_high] = dec.predictBayesDecoder(spkRate(t_high,:), 0,'mean');
    ...
    Posterior_low  = (2.^Posterior_low)/50;
    Posterior_high = (2.^Posterior_high)/50;
    Posterior_gray = (2.^Posterior_gray)/50;
...
%         Posterior_norm_orig_d(Posterior_norm_orig_d<-5.6) = -5.6;
        Posterior_low_orig_d  = (2.^Posterior_low_orig_d)/50;
        Posterior_high_orig_d = (2.^Posterior_high_orig_d)/50;    
        Posterior_norm_orig_d = (2.^Posterior_norm_orig_d)/50;
    else
%         tmpDec_low = dec_l;
%         tmpDec_high = dec_h;
        ...
        for icell = 1:size(spkRate,2)
            ...
            [tmpML_low ] = dec.predictBayesDecoder(spkRate(t_low,:), 0,'mean');
            [tmpML_high] = dec.predictBayesDecoder(spkRate(t_high,:), 0,'mean');
            [tmpML_norm] = dec.predictBayesDecoder(spkRate(t,:), 0,'mean');
