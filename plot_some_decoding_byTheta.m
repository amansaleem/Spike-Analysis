function [Posterior_all] = plot_some_decoding_byTheta(animal, iseries,...
...
%     Posterior_high(Posterior_high<-5.6) = -5.6;
    ...
    [dec_l, ~, X_low_orig, Posterior_low_orig, ~, nonNormPosterior_low_orig] = ...
        dec_l.trainDecoder( es.traj(t_low), spkRate(t_low,:), 0);
    [dec_h, ~, X_high_orig, Posterior_high_orig, ~, nonNormPosterior_high_orig] = ...
        dec_h.trainDecoder( es.traj(t_high), spkRate(t_high,:), 0);
...
%         tmpDec_high = dec_h;
        ...
        for icell = 1:size(spkRate,2)
            tmpSpkRate = spkRate;
            tmpSpkRate(:,icell) = 0;
            [tmpML_norm] = dec.predictBayesDecoder(tmpSpkRate(t,:), 0,'mean');
            
            [tmpML_low_norm ] = dec.predictBayesDecoder(tmpSpkRate(t_low,:), 0,'mean');
            [tmpML_high_norm] = dec.predictBayesDecoder(tmpSpkRate(t_high,:), 0,'mean');
            
            [tmpML_low_orig ] = dec_l.predictBayesDecoder(tmpSpkRate(t_low,:), 0,'mean');
            [tmpML_high_orig] = dec_h.predictBayesDecoder(tmpSpkRate(t_high,:), 0,'mean');
            
            if mean((tmpML_low_orig - X_low_orig').^2) < mean((tmpML_low_norm - X_low_orig').^2)
                tmpML_low = tmpML_low_orig;
            else
                tmpML_low = tmpML_low_norm;
            end
            if mean((tmpML_high_orig - X_high_orig').^2) < mean((tmpML_high_norm - X_high_orig').^2)
            ...
            Posterior_all.MAP.dec.low(icell,:)  = tmpML_low;
            Posterior_all.MAP.dec.norm(icell,:) = tmpML_norm;
            Posterior_all.MAP.dec.high(icell,:) = tmpML_high;
            ...
            [tmpMap_low] = tmpMap_low.trainSpikeMap(tmpML_low(X_low_orig<rewlim)', spkRate(t_low & temp<rewlim,icell), 0);
