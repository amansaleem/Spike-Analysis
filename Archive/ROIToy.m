classdef ROIToy < handle
  ...
  properties (SetAccess = private)
    ...
    StackIdx = 1
    NumRegions = 0
    Regions = struct('x', {}, 'y', {}, 'mask', {}, 'outlinePoints', {})
    IsPlaying = false
    ViewMode = 'frame'
    PlayStep
  end
  ...
  methods
    function obj = ROIToy(parent)
      obj.BlankStack = img.ImageArray(zeros(100));
  ...
  methods (Access = private)
    ...
    function idx = newRegion(obj, x, y, r)
      ...
      % create a mask
      d = obj.Stacks(obj.StackIdx).FrameSize;
      region.mask = circleMask(r, x, y, d(2), d(1));
    ...
    function frameDataChanged(obj)
      ...
      range = increasing([obj.Stacks(currStack).Min obj.Stacks(currStack).Max]);
